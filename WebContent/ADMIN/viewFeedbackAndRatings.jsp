...
<title>PTT Search Feedback</title>
				...
				<div id="page-content" style="min-height: 800px">
					<div class="content-header">
						<div class="header-section">
							<h1>
								<i class="fa fa-table"></i>Manage Feedback<br>
							</h1>
						</div>
					</div>
					<ul class="breadcrumb breadcrumb-top">
						...
						<li><a href="Search_feedback.jsp">View Feedback And Ratings</a></li>
					</ul>
					<div class="block full">
						<div class="block-title">
							<h2>
								<strong>View</strong> Feedback And Ratings
							</h2>
						</div>
							...
							<table id="example-datatable"
								...
								<thead>
									<tr>
										...
										<th class="text-center">Parkinglot Name</th>
								...
								<c:if test="${sessionScope.feedbackAndRatingListPP ne null }">
								<c:forEach items="${sessionScope.feedbackAndRatingListPP}" var="i">
							
									<tr>
										<td class="text-center">${i.loginVO.email}</td>
										<td class="text-center">${i.parkinglotsVO.val_plotname2}</td>
										<td class="text-center">${i.feedback}</td>
										<td class="text-center">${i.rating}</td>
										<td class="text-center">
										<div class="btn-group">
										<a href="<%=request.getContextPath() %>/FeedbackController?flag=deleteFeedback&id=${i.feedback_id}" data-toggle="tooltip" title="Delete"class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>
										</div>
								
									</tr>
								</c:forEach>	
								</c:if>
								<c:if test="${sessionScope.feedbackAndRatingListPP eq null }">
								<c:forEach items="${sessionScope.feedbackAndRatingList}" var="j">
							
									<tr>
										<td class="text-center">${j.loginVO.email}</td>
										<td class="text-center">${j.parkinglotsVO.val_plotname2}</td>
										<td class="text-center">${j.feedback}</td>
										<td class="text-center">${j.rating}</td>
										<td class="text-center">
										<div class="btn-group">
										<a href="<%=request.getContextPath() %>/FeedbackController?flag=deleteFeedback&id=${j.feedback_id}" data-toggle="tooltip" title="Delete"class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>
										</div>
									</tr>
								</c:forEach>	
								</c:if>
