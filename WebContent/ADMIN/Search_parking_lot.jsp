...
<title>PTT Search Parking Lot</title>
				...
				<div id="page-content" style="min-height:800px">
					...
					<div class="block full">
						<div class="block-title">
							<h2>
								<strong>Search</strong> Parking lots
							</h2>
						</div>
						<div class="table-responsive">
							<table id="example-datatable"
								class="table table-vcenter table-condensed table-bordered">
								<div style="color:red"><b>${sessionScope.errorMsg}</b></div>
								<thead>
									<tr>
										<th class="text-center">Parking lot ID</th>
										...
										<th class="text-center">Two Wheeler Price</th>
										<th class="text-center">Four Wheeler Capacity</th>
										<th class="text-center">Four Wheeler Price</th>							
										
										<th class="text-center">Address</th>
										<th class="text-center">Description</th>
										<c:if test="${sessionScope.ParkingproviderID ne null}">
										<th class="text-center">Actions</th>
										</c:if>
									</tr>
								</thead>
								<tbody>
								<c:if test="${sessionScope.ParkingLotsOfProvider ne null }">
								<c:forEach items="${sessionScope.ParkingLotsOfProvider}" var="i">     
								<tr>
										<td class="text-center">${i.parkinglots_id}</td>
										<td class="text-center">${i.val_plotname2}</td>
										<td class="text-center">${i.val_latitude}</td>
										<td class="text-center">${i.val_longitude}</td>
										<td class="text-center">${i.val_contactpersonname}</td>
										<td class="text-center">${i.val_contactnumber}</td>
										<td class="text-center">${i.val_2capacity}</td>
										<td class="text-center">${i.val_2price}</td>
										<td class="text-center">${i.val_4capacity}</td>
										<td class="text-center">${i.val_4price}</td>
										
										<td class="text-center">${i.val_address}</td>
										<td class="text-center">${i.val_description}</td>
										
										<td class="text-center">
											<div class="btn-group">
												<a href="${pageContext.request.contextPath}/ParkinglotsController?flag=editLot&parkinglot_id=${i.parkinglots_id}" data-toggle="tooltip"
													title="Edit" class="btn btn-xs btn-default"><i
													class="fa fa-pencil"></i></a> <a href="${pageContext.request.contextPath}/ParkinglotsController?flag=deleteLot&parkinglot_id=${i.parkinglots_id}"
									...
									<c:if test="${sessionScope.ParkingLotsOfProvider eq null }">
									<c:forEach items="${sessionScope.searchLotAdmin}" var="x">     
								<tr>
										<td class="text-center">${x.parkinglots_id}</td>
										<td class="text-center">${x.val_plotname2}</td>
										<td class="text-center">${x.val_latitude}</td>
										<td class="text-center">${x.val_longitude}</td>
										<td class="text-center">${x.val_contactpersonname}</td>
										<td class="text-center">${x.val_contactnumber}</td>
										<td class="text-center">${i.val_2capacity}</td>
										<td class="text-center">${i.val_2price}</td>
										<td class="text-center">${i.val_4capacity}</td>
										<td class="text-center">${i.val_4price}</td>
										
										<td class="text-center">${x.val_address}</td>
										<td class="text-center">${x.val_description}</td>
										<%-- <td class="text-center">
											<div class="btn-group">
												<a href="${pageContext.request.contextPath}/ParkinglotsController?flag=editLot&parkinglot_id=${x.parkinglots_id}" data-toggle="tooltip"
													title="Edit" class="btn btn-xs btn-default"><i
													class="fa fa-pencil"></i></a> <a href="${pageContext.request.contextPath}/ParkinglotsController?flag=deleteLot&parkinglot_id=${x.parkinglots_id}"
													data-toggle="tooltip" title="Delete"
													class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>
											</div>
										</td> --%>
